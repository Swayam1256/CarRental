...
const Dashboard = () => {
  const { axios, isOwner, currency, navigate, token, logout } = useAppContext();
  ...
  // ✅ Fetch owner dashboard (with token header)
  ...
  }, [isOwner, token, fetchDashboardData, navigate]);
      ...
      <div className="flex flex-wrap items-start gap-6 mb-8 w-full">
        ...
        <div className="p-4 md:p-6 mb-6 border border-borderColor rounded-md w-full md:max-w-xs">
          ...
          <p className="text-3xl mt-6 font-semibold text-primary">
            {currency}{" "}
            {isNaN(data.monthlyRevenue)
              ? "0.00"
              : Number(data.monthlyRevenue).toFixed(2)}
          </p>
        </div>
      </div>
    </div>
